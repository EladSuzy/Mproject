...
1.  **Ingress**: 
    *   User traffic hits the **Global External Load Balancer**.
    *   **Cloud Armor** inspects traffic for DDoS/Attacks.
    *   Valid requests are forwarded to the **Cloud Run** service via a **Serverless NEG**.

2.  **Processing**:
    *   **Cloud Run** spins up containers based on traffic (Autoscaling).
    *   Secrets (DB password) are mounted at runtime from **Secret Manager**.

3.  **Data Access**:
    *   The application connects to **Cloud SQL** using its **Private IP**.
    *   Traffic flows through the **Serverless VPC Connector**, ensuring it never hits the public internet.

4.  **Observability**:
    *   Logs are automatically sent to **Cloud Logging**.
    *   Metrics (latency, errors) are aggregated in **Cloud Monitoring**.
    *   High error rates trigger an **Alerting Policy**.
